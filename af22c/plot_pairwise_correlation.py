...
def plot_pairwise_correlation(ax: plt.Axes, datasets: list[list[float]], labels: list[str]):
    """
    ...
    if len(datasets) != len(labels):
        raise ValueError(f"number of datasets ({len(datasets)}) does not match number of labels ({len(labels)}")
